...
OEVENTLUA[30050] = function() --奇遇总汇
	local pd = math.modf((101 - JY.Person[0]["资质"]) * 0.2)
	local qycs = GetS(106, 63, 0, 0)+1--一剑
	local sycs = math.modf(((101 - JY.Person[0]["资质"]) * 0.2)+ 10 * JY.Base["天书数量"])-GetS(106, 63, 0, 0)--一剑
	if GetS(106, 63, 0, 0) >= math.modf(pd + 1 + 10 * JY.Base["天书数量"]) then
		do return end
	end
	local yes = 0
	...
	if yes == 1 then
		...
		say("已使用奇遇次数："..qycs.."次，剩余奇遇次数："..sycs.."次",0,4)--一剑
	end	
end
...
OEVENTLUA[30051] = function(aa) --捡物品
	Cls()
	say("咦？地上好像有什么东西....",0,4)
	DrawStrBoxYesNo(-1, -1, "要捡物品吗？", C_WHITE, 30) --一剑	
...
OEVENTLUA[30052] = function(aa, bb) --捡原料
    Cls()
	say("咦？地上好像有什么东西....",0,4)
	DrawStrBoxYesNo(-1, -1, "要捡原料吗？", C_WHITE, 30) --一剑	
...
OEVENTLUA[30053] = function() --挑战
	...
	AddPersonAttrib(prot, "实战", 30)
	QZXS(JY.Person[prot]["姓名"].."实战增加30点！")
	say("哈哈！赚到了赚到了！",0,4)
end
...
OEVENTLUA[30056] = function() --遇铁匠
	...
	Cls()
	local aa, bb
	aa = math.random(1, 3) * 3
	bb = aa
	JY.Thing[r]["加攻击力"] = limitX(JY.Thing[r]["加攻击力"] + aa,1,1000)
	QZXS(JY.Thing[r]["名称"].."增加攻击力".. bb.."点")
	aa = math.random(3, 5) * 3
	bb = aa
	JY.Thing[r]["加防御力"] = limitX(JY.Thing[r]["加防御力"] + aa,1,1000)
	QZXS(JY.Thing[r]["名称"].."增加防御力".. bb.."点")	
	aa = math.random(3, 5) * 3
	bb = aa
	JY.Thing[r]["加轻功"] = limitX(JY.Thing[r]["加轻功"] + aa,1,1000)
...
OEVENTLUA[30057] = function(aa, bb) --洗武功
	...
	while hasit == 1 do
		...
		if wugong == 100 or wugong == 102 or wugong == 281 or wugong == 367 then
			wogong = wugong - 1
		end
		if wugong == 160 or wugong == 203 or wugong == 282 or wugong == 359 or wugong == 367 then
			wogong = wugong - 2
		end
		if wugong == 171 then
			wugong = 166
