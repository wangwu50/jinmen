...
x=math.random(15,19)
y=math.random(33,36)
elseif DD==1 then --河洛客栈
x=math.random(3,18)
y=math.random(29,39)
elseif DD==3 then --有间客栈
x=math.random(17,31)
y=math.random(28,46)
...
elseif CX==603 then --鳌拜
t1=math.random(4764,4765)
zdbh=260

elseif CX==648 then --鹿杖客
t1=math.random(4632,4635)
zdbh=295

elseif CX==647 then --鹤笔翁
t1=math.random(4636,4639)
zdbh=295
elseif CX==616 then --公孙止
t1=math.random(4355,4358)
zdbh=271
elseif CX==602 then --海大富
t1=4149
zdbh=259
elseif CX==595 then --宝象
t1=math.random(4488,4491)
zdbh=246
elseif CX==594 then --其长发
t1=2948
zdbh=247
...
local y=GetS(92,60,19,5)
 ...
 --say("测试地图ID"..zdbh,0,1)
	...
	if JY.Person[0]['性别'] == 0 then
		say(string.format("Ｇ%s！你作恶多端，今日小爷来为民除害，纳命来！",JY.Person[CX]["姓名"]),0,1)
	else
		say(string.format("Ｇ%s！你作恶多端，今日姑奶奶来为民除害，纳命来！",JY.Person[CX]["姓名"]),0,1)	
	end	
    ...
    instruct_13();
	    ...
	    DrawStrBoxWaitKey(string.format("您打跑了%s，恭喜您获得%d兩银子！",JY.Person[CX]["姓名"],money),C_WHITE,CC.DefaultFont);
	  	...
	  	for j = 1,JY.Base['武功数量'] do
	  		if JY.Person[CX]['武功'..j] > 0 then
	  			wg[#wg+1] = JY.Person[CX]['武功'..j]
	  		end
	  	end
	  	if wg ~= nil and #wg > 0 then
	  		local n = wg[math.random(#wg)]
	  		for k = 0, JY.ThingNum-1 do	
	  			if JY.Thing[k]['练出武功'] == n then
	  				instruct_2(k,1)
	  			end
	  		end
	  	end]]				
   	...
   	TalkEx("本蟾不发威，你当我是汉堡啊，暂且饶你狗命！",216,1)
		...
		elseif CX==602 then
		say("不知死活的东西，杀了你玷污了老子的名声，暂且饶你狗命！",602,1,"海大富")
		elseif CX==599 then
